...
export default function DashboardSidebar() {
  ...
  const normalStyle = 'sidebar overflow-y-auto overflow-x-auto fixed bg-[#31353d] text-gray-500 z-50 h-full shadow-lg shadow-gray-900/20 transition duration-300 ease-in-out w-[20rem]'
  ...
  return (
    <aside className={`${normalStyle} ${checkCollapsedStyle}`}>
      <div className='sidebar-top flex relative items-center py-5 px-3.5 gap-1'>
        ...
        {!toggleCollapse && <h3 className='logo-title pl-2 font-serif text-2xl text-[#e6e9ee] min-w-max'>
          Batcave
        </h3>}
      </div>
